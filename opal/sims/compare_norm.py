...
# OpAL model (without dopamine modulation)
#
# Name: opal/compare_norm.py
##################################################################
...
def main(env,reprocess):
	...
	# which k to use
	path_normT = "results/grid_%s/%s/norm_%s/k_0.0/mod_constant/" \
				%(ext,env,"True")
	path_normF = "results/grid_%s/%s/norm_%s/k_0.0/mod_constant/" \
				%(ext,env,"False")
	...
	if os.path.exists(dump_it) and not reprocess:
		...
		for par in param_comb:
			...
			if os.path.exists(path_normT + "params_" + str(par) + ".pkle"):
				# get normalized data
				auc_learnT, auc_rewardT, learn_curve, reward_curve = \
				pickle.load(open(path_normT + "params_" + str(par) + ".pkle","rb"))
				# get unnormalized data 
				auc_learnF, auc_rewardF, learn_curve, reward_curve = \
				pickle.load(open(path_normF + "params_" + str(par) + ".pkle","rb"))

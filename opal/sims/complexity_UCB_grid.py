...
def main(UCB_gamma,env_base,n_trials,n_states):
    """ 
    ...
    UCB_gamma = UCB_gamma/100. # rescale to int
    print("Gamma")
    print(UCB_gamma)

    root_me = "revisions2/UCB/"
    ...
    for level in np.arange(n_levels):
        ...
        if os.path.exists(path + seed_ext):
            print("path")
            full_seed_path = seed_path + seed_ext
            print(full_seed_path)
            rnd_seeds = pickle.load(open(full_seed_path,"rb"))
        ...
        states = learn.simulate((0.05,5.),n_states,n_trials,v0=v0,env=env,r_mag=r_mag,l_mag=l_mag,rnd_seeds=rnd_seeds,\
            UCB = True, UCB_samplemean=True, gamma = UCB_gamma)
        ...
        pickle.dump([auc_learn, auc_reward, learn_curve, reward_curve, rnd_seeds], \
                open(path_ext + "params_%s.pkle" %(str(UCB_gamma)),"wb"))

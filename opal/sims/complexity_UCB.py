...
def get_opt_params(n_options,n_trials):

    opt_path = "../../optimize/res/noptions%d/ntrials_%d/RL_['1', '1'].pkle" %(n_options,n_trials)
    res = pickle.load(open(opt_path,"rb"))
    params = res.x[0]

    return params

def main(env_base,n_trials,n_states):
    """ 
    ...
    for level in np.arange(n_levels):
        ...
        # get the optimized params for the nstates/ntrials
        opt_gamma = get_opt_params(level,n_trials)
        temp_pars = np.array([0.05,10.])

        # learn the things
        # params,n_states,n_trials,v0=0.0,env = "rich",r_mag = 1, l_mag = 0, rnd_seeds = None
        states = learn.simulate(temp_pars,n_states,n_trials,v0=v0,env=env,r_mag=r_mag,l_mag=l_mag,rnd_seeds=rnd_seeds,\
            UCB = True, UCB_samplemean=True, gamma = opt_gamma)
